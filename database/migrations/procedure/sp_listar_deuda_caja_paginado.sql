...
begin
	...
	If v_count::Integer > p_limit::Integer then
		v_scad:='SELECT '||v_campos||v_col_count||v_tabla||v_where||' group by a.numero_cap, apellidos_nombre ' ||' Order By apellidos_nombre asc LIMIT '||p_limit||' OFFSET '||p_pagina||';'; 
	else
		v_scad:='SELECT '||v_campos||v_col_count||v_tabla||v_where||' GROUP BY a.numero_cap, apellidos_nombre ' ||' Order By apellidos_nombre asc;'; 
	End If;
