...
begin
	...
	v_campos=' c.id, r.denominacion regional, c.codigo , c.denominacion , tc.denominacion tipo_concepto, c.importe, tm2.denominacion id_moneda, c.periodo, pc.cuenta cuenta_contable_debe, pc2.cuenta cuenta_contable_al_haber1, pc2.cuenta cuenta_contable_al_haber2 , pp.codigo partida_presupuestal , tm.denominacion tipo_afectacion, cc.codigo centro_costo , c.estado';

	v_tabla='from conceptos c
	left join regiones r on c.id_regional = r.id
	left join plan_contables pc on c.cuenta_contable_debe::int = pc.id
	left join plan_contables pc2 on c.cuenta_contable_al_haber1 ::int = pc.id
	left join plan_contables pc3 on c.cuenta_contable_al_haber2 ::int = pc.id
