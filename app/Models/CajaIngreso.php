...
*/
    ...
    function getAllMovimientoComprobantes($id_usuario, $id_caja, $f_inicio, $f_fin, $tipo){
        ...
        $cad = "
                    select co.denominacion concepto,c.fecha fecha,	 c.tipo tipo_documento, c.serie serie, c.numero::varchar(20)  numero, c2.fecha fecha_ncd,c2.tipo tipo_documento_ncd, c2.serie serie_ncd, c2.numero::varchar(20)  numero_ncd, c.cod_tributario cod_tributario, c.destinatario destinatario ,case when co.id_tipo_afectacion=30 then 0 else  c.subtotal end * case when c.tipo='NC' then -1 else 1 end  imp_afecto,case when co.id_tipo_afectacion=30 then c.subtotal else 0  end * case when c.tipo='NC' then -1 else 1 end imp_inafecto,igv_total* case when c.tipo='NC' then -1 else 1 end igv  ,c.total * case when c.tipo='NC' then -1 else 1 end total
                    from (
                            select select co.denominacion concepto,c.fecha fecha,	 c.tipo tipo_documento, c.serie serie, c.numero::varchar(20)  numero, c2.fecha fecha_ncd,c2.tipo tipo_documento_ncd, c2.serie serie_ncd, c2.numero::varchar(20)  numero_ncd, c.cod_tributario cod_tributario, c.destinatario destinatario ,case when co.id_tipo_afectacion=30 then 0 else  c.subtotal end * case when c.tipo='NC' then -1 else 1 end  imp_afecto,case when co.id_tipo_afectacion=30 then c.subtotal else 0  end * case when c.tipo='NC' then -1 else 1 end imp_inafecto,igv_total* case when c.tipo='NC' then -1 else 1 end igv  ,c.total * case when c.tipo='NC' then -1 else 1 end total
                            from comprobantes c 
                                inner join comprobante_detalles cd on cd.id_comprobante =c.id
