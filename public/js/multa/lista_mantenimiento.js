...
function datatablenew(){
    var oTable1 = $('#tblAfiliado').dataTable({
        "bServerSide": true,
        "sAjaxSource": "/multa/listar_multa_ajax",
        "bProcessing": true,
        ...
        "fnServerData": function (sSource, aoData, fnCallback, oSettings) {
            ...
            oSettings.jqXHR = $.ajax({
                ...
                "data":{NumeroPagina:iNroPagina,NumeroRegistros:iCantMostrar,
						id:id,denominacion:denominacion,monto:monto,moneda:moneda,estado:estado,
						_token:_token
                       },
                "success": function (result) {
                    fnCallback(result);
            ...
            [	
				...
				},
				{
					"mRender": function (data, type, row) {
						...
						html += '<button style="font-size:12px" type="button" class="btn btn-sm btn-success" data-toggle="modal" onclick="modalMultaMantenimiento('+row.id+')" ><i class="fa fa-edit"></i> Editar</button>';
...
function modalMultaMantenimiento(id){
	
	$(".modal-dialog").css("width","85%");
	$('#openOverlayOpc .modal-body').css('height', 'auto');

	$.ajax({
			url: "/multa/modal_multa_nuevoMultaMantenimiento/"+id,
			type: "GET",
			success: function (result) {  
